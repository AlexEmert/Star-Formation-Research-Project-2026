...
## In this script, I take the data from the original text files that they were in, drop flux density photometry values based on their respective problematic flags (those being Saturation, no source, and poor confidence), and combine the photometry data with the cross matched longer wavelength fluxes. This extensive dataframe is save as `MIRION_cleaned_all_fluxes.csv`
...
## added 6/25/26
## looking at if dropping those columns correspond to negative uncertainties

## if old MIRION_cleaned_everything.csv is needed, remove this cell
```{python}
...
```{python}
phot_extra_cleaned.to_csv('MIRION_phot_fluxes.csv', index=False)
```
...
## loading uncertainties from wades csv
