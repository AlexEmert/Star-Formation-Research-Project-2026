...
phot = phot.drop(columns=['LRATIO', 'SURF_DENS', 'LM', 'L_BOL', 'MASS', 'DIAM', 'TEMP', 'YB'])
...
log_phot_y = np.log1p(phot['T_BOL'])
...
for name, setup in models.items():
    print(f"--- Running {name} for log of tbol---")
    ...
    print(f"Best Score with log tbol (not translatable): {-log_opt.best_score_:.4f}")
    print(f"Best Params with log tbol (not translatable): {dict(log_opt.best_params_)}\n")
