...
phot = phot.drop(columns=['LM', 'T_BOL', 'SURF_DENS', 'L_BOL', 'MASS', 'DIAM', 'TEMP', 'YB'])
...
for name, setup in models.items():
    print(f"--- Running {name} for lratio---")
...
log_phot_y = np.log1p(phot['LRATIO'])
...
for name, setup in models.items():
    print(f"--- Running {name} for log of lratio---")
