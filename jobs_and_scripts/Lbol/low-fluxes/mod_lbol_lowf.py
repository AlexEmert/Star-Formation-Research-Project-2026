...
phot = phot.drop(columns=['LM', 'T_BOL', 'SURF_DENS', 'DIAM', 'LRATIO', 'MASS', 'TEMP', 'YB'])
...
log_phot_y = np.log1p(phot['L_BOL'])
...
for name, setup in models.items():
    print(f"--- Running {name} for log of Lbol---")
